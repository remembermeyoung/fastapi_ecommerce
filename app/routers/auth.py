...
async def create_access_token(username: str, user_id: int, is_admin: bool, is_supplier: bool, is_customer: bool,
    ...
    payload = {
        ...
        'exp': dt.now(timezone.utc) + expires_delta
    }
...
async def authenticate_user(db: Annotated[AsyncSession, Depends(get_db)], username: str, password: str):
    ...
    if not user or bcrypt_context.verify(password, user.hashed_password) or user.is_active is False:
        raise HTTPException(
...
@user_router.get('/read_current_user')
